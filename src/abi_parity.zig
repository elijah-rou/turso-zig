...
pub const functions = [_]FunctionRow{
    ...
    row("turso_connection_enable_load_extension", .trust_boundary_adapter, &.{.connection_set_extension_loading}, "Explicit unsafe capability grant to every SQL submitter on the connection.", "unsafe native-code trust boundary"),
    ...
    row("turso_statement_deinit", .internal_ownership, &.{.statement_deinit}, "Owning teardown that can cancel and drain pending native work.", "none"),
    ...
    row("turso_statement_reset", .safe_wrapper, &.{.statement_reset}, "Explicit cancellation that may drain pending native work.", "none"),
...
fn apiName(api: ZigApi) []const u8 {
    return switch (api) {
        ...
        .connection_set_extension_loading => "Connection.setSqlExtensionLoadingEnabledUnsafe",
...
fn validateApiReference(api: ZigApi) void {
    switch (api) {
        ...
        .connection_set_extension_loading => assertExactFunction(Connection.setSqlExtensionLoadingEnabledUnsafe, fn (*Connection, bool) errors.Error!void, "Connection.setSqlExtensionLoadingEnabledUnsafe"),
